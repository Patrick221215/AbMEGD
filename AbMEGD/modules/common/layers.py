...
class GaussianRBFExpansion(nn.Module):
    """
    SchNet-style Gaussian radial basis expansion.

    This is intentionally different from ExpNormalSmearing:
      - centers are uniformly spaced in [start, cutoff]
      - no exponential warping
      - no directional information
    """

    def __init__(self, cutoff=5.0, num_rbf=50, trainable=False, start=0.0):
        super().__init__()
        ...
        if self.cutoff <= self.start:
            raise ValueError("cutoff must be larger than start")

        centers, gamma = self._initial_params()
    ...
    def _initial_params(self):
        centers = torch.linspace(self.start, self.cutoff, self.num_rbf)
        if self.num_rbf == 1:
            gap = self.cutoff - self.start
        else:
            gap = (self.cutoff - self.start) / (self.num_rbf - 1)
        gamma = torch.full((self.num_rbf,), 1.0 / max(gap, 1e-6) ** 2)
