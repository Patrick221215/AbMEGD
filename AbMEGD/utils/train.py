...
def log_losses(out, it, tag, logger=BlackHole(), writer=BlackHole(), others=None):
    ...
    overall = _scalar_to_float(out['overall'])
    logstr += ' | loss %.4f' % overall

    for k, v in out.items():
        if k == 'overall':
            continue
        logstr += ' | loss(%s) %.4f' % (k, _scalar_to_float(v))

    for k, v in others.items():
        logstr += ' | %s %2.4f' % (k, _scalar_to_float(v))

    logger.info(logstr)

    for k, v in out.items():
        if k == 'overall':
            writer.add_scalar('%s/loss' % tag, _scalar_to_float(v), it)
        else:
            writer.add_scalar('%s/loss_%s' % (tag, k), _scalar_to_float(v), it)

    for k, v in others.items():
        writer.add_scalar('%s/%s' % (tag, k), _scalar_to_float(v), it)
